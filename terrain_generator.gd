...
func spawn_terrain():
	...
	for i in range(10):
		...
		if player:
			next_x = max(next_x, player.global_position.x)
		next_x += randi_range(min_width, max_width*max(1, 3*log(placed/10)))
		placed += 1
...
func clear_old_terrain():
	...
	for child in get_children():
		if child.is_class("Timer"):
			continue
		if child.global_position.x < (player.global_position.x - 10000):
			child.queue_free()
